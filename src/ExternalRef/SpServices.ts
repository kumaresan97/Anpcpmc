...
const SPCreateLibraryFolder = async (params: { folderPath: string }) => {
  try {
    const res = await sp.web.folders.addUsingPath(params?.folderPath);
    return res;
  } catch (err) {
    console.log("createLibraryFolder err", err);
  }
};
